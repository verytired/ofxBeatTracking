...
ofxBeatTracking::ofxBeatTracking()
{
    //int fft_size=512;
    //int buffer_size = 1024;
...
void ofxBeatTracking::updateFFT()
{
    if(fftInit) {
        //fft = in_fft;
...
void ofxBeatTracking::audioReceived(float* input, int bufferSize){
...
// ビートそのものがあったか
bool ofxBeatTracking::isBeat(int subband)
{
    return fftSubbands[subband] > averageEnergy[subband]*beatValueArray[subband];
}

bool ofxBeatTracking::isKick()
{
    return isBeat(0);
}

bool ofxBeatTracking::isSnare()
{
    int low = 1;
...
bool ofxBeatTracking::isBeatRange(int low, int high, int threshold)
{
    int num = 0;
